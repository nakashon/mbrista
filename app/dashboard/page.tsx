...
import {
  ...
  Thermometer, Droplets, Weight, Smartphone, Maximize2, ChevronRight, X,
} from "lucide-react";
import {
...
function LiveChartSection({
  ...
  const phase = liveStatus?.name ?? liveStatus?.state ?? "idle";
  ...
  return (
    <div className="space-y-3">
      {/* Live chart card */}
      <div className="rounded-2xl border border-[#e8944a]/20 bg-[#161210] p-5">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <span className="h-2 w-2 rounded-full bg-red-500 animate-pulse" />
            <p className="text-sm font-semibold text-[#f5f0ea]">Live Extraction</p>
            {data.length > 0 && (
          ...
          <button
            onClick={onExpand}
            title="Focus mode"
            className="inline-flex items-center gap-1.5 rounded-xl border border-white/[0.08] bg-transparent px-3 py-1.5 text-xs font-medium text-[#f5f0ea]/50 hover:bg-white/[0.05] hover:text-[#f5f0ea] transition-all"
          >
        ...
        {data.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-40 gap-3">
            <Loader2 className="h-5 w-5 animate-spin text-[#e8944a]" />
            <p className="text-sm text-[#f5f0ea]/40">Waiting for data…</p>
          </div>
        ) : (
    ...
    connectSocket({
      ...
      status:     (s) => {
        ...
        if (s.extracting) {
          ...
          setLiveData((prev) => [...prev.slice(-300), point]);
        } else {
  ...
  return (
    <div className="min-h-screen bg-[#0c0a09]">
      <div className="mx-auto max-w-5xl px-6 py-10 space-y-6">
        ...
        {/* Live chart section — only shown when extracting */}
        {isExtracting && (
          <LiveChartSection
            data={liveData}
            liveStatus={liveStatus}
            onExpand={() => router.push("/live")}
          />
        )}

        {/* Last shot summary banner */}
        {shotSummary && !summaryDismissed && !liveStatus?.extracting && (
